...
def criar_banco():
    ...
    # Reels: ordem correta com nome_video em index 3
    ...
    # Migração para bancos antigos com nome_video na posição errada
    try:
        c.execute('ALTER TABLE metricas_reels ADD COLUMN nome_video VARCHAR(100)')
    except Exception:
        pass
...
# ─── SETUP (primeiro acesso — sem usuários no banco) ──────────────────
...
def login():
    ...
    erro  = None
    aviso = request.args.get('setup') or request.args.get('senha_redefinida')
    ...
    if request.method == 'POST':
        ...
        if not nome or not email or not senha:
            ...
            try:
                ...
                conn.commit(); conn.close()
                sucesso = 'Conta criada! Faça login para entrar.'
            except sqlite3.IntegrityError:
                erro = 'Este e-mail já está cadastrado.'
    return render_template('cadastro.html', erro=erro, sucesso=sucesso)


# ─── REDEFINIR SENHA (sem email — link direto) ────────────────────────
...
def esqueci_senha():
    ...
    if request.method == 'POST':
        ...
        if u:
            ...
            conn.commit()
            conn.close()
            # Sem email configurado: redireciona direto para redefinir
            return redirect(f'/redefinir-senha/{token}')
        conn.close()
        resultado = 'Se este e-mail estiver cadastrado, o link será gerado.'
...
def redefinir_senha(token):
    ...
    if request.method == 'POST':
        ...
        if len(senha) < 6:
            ...
            conn = sqlite3.connect('banco.db')
            conn.execute('UPDATE usuarios SET senha_hash=?, token_verif=NULL WHERE id=?',
                         (hash_senha(senha), u[0]))
            conn.commit(); conn.close()
...
def cliente(id):
    ...
    cur.execute('SELECT * FROM metricas_meta   WHERE cliente_id=? ORDER BY data DESC', (id,))
    meta = cur.fetchall()
    cur.execute('SELECT * FROM metricas_google WHERE cliente_id=? ORDER BY data DESC', (id,))
    google = cur.fetchall()
    cur.execute('SELECT * FROM metricas_reels  WHERE cliente_id=? ORDER BY data DESC', (id,))
    reels = cur.fetchall()
    cur.execute('SELECT * FROM metas WHERE cliente_id=?', (id,))
    metas = cur.fetchone()
    conn.close()
    return render_template('cliente.html', cliente=cliente, meta=meta,
                           google=google, reels=reels, metas=metas)
...
def inserir(id):
    ...
    if request.method == 'POST':
        ...
        conn  = sqlite3.connect('banco.db'); cur = conn.cursor()
        if canal == 'meta':
            cur.execute('''INSERT INTO metricas_meta
            ...
            cur.execute('''INSERT INTO metricas_google
            ...
            cur.execute('''INSERT INTO metricas_reels
                (cliente_id,data,nome_video,views,retencao,alcance,curtidas,
...
def review(id):
    ...
    if semana_atual and semanas:
        ...
        cur.execute('SELECT * FROM metricas_meta   WHERE cliente_id=? AND data=?', (id, semana_atual))
        meta_semana = cur.fetchone()
        cur.execute('SELECT * FROM metricas_google WHERE cliente_id=? AND data=?', (id, semana_atual))
        google_semana = cur.fetchone()
        cur.execute('SELECT * FROM metricas_reels  WHERE cliente_id=? AND data=?', (id, semana_atual))
        reels_semana = cur.fetchall()
        if sem_ant:
            cur.execute('SELECT * FROM metricas_meta   WHERE cliente_id=? AND data=?', (id, sem_ant))
            meta_ant = cur.fetchone()
            cur.execute('SELECT * FROM metricas_google WHERE cliente_id=? AND data=?', (id, sem_ant))
            google_ant = cur.fetchone()
            cur.execute('SELECT * FROM metricas_reels  WHERE cliente_id=? AND data=?', (id, sem_ant))
            reels_ant = cur.fetchall()
    cur.execute('SELECT * FROM metricas_meta   WHERE cliente_id=? ORDER BY data DESC', (id,))
    meta = cur.fetchall()
    cur.execute('SELECT * FROM metricas_google WHERE cliente_id=? ORDER BY data DESC', (id,))
    google = cur.fetchall()
    ...
    if reels_semana:
        ac += 2 if float(reels_semana[0][4] or 0) < 25 else (1 if float(reels_semana[0][4] or 0) < 40 else 0)
    if not meta_semana and not google_semana and not reels_semana:
        sc, st = 'status-neutro', 'Sem dados'
    elif ac >= 3: sc, st = 'status-critico',  '🔴 Crítico'
    elif ac >= 1: sc, st = 'status-atencao',  '🟡 Atenção'
    else:         sc, st = 'status-saudavel', '🟢 Saudável'
    dados_ia = {}
    if meta_semana:
...
def relatorio(id):
    ...
    semanas_do_periodo = ([s for s in semanas if s.startswith(periodo_atual[:7])]
                          ...
                          else ([periodo_atual] if periodo_atual else []))
    meta_periodo = google_periodo = reels_periodo = []
    if semanas_do_periodo:
        ph = ','.join(['?']*len(semanas_do_periodo))
        cur.execute(f'SELECT * FROM metricas_meta   WHERE cliente_id=? AND data IN ({ph}) ORDER BY data', [id]+semanas_do_periodo)
        meta_periodo = cur.fetchall()
        cur.execute(f'SELECT * FROM metricas_google WHERE cliente_id=? AND data IN ({ph}) ORDER BY data', [id]+semanas_do_periodo)
        google_periodo = cur.fetchall()
        cur.execute(f'SELECT * FROM metricas_reels  WHERE cliente_id=? AND data IN ({ph}) ORDER BY data', [id]+semanas_do_periodo)
        reels_periodo = cur.fetchall()
    cur.execute('SELECT * FROM metricas_meta   WHERE cliente_id=? ORDER BY data', (id,))
    historico_meta = cur.fetchall()
    cur.execute('SELECT * FROM metricas_google WHERE cliente_id=? ORDER BY data', (id,))
    historico_google = cur.fetchall()
    cur.execute('SELECT * FROM metricas_reels  WHERE cliente_id=? ORDER BY data', (id,))
    historico_reels = cur.fetchall()
    conn.close()
    ...
    melhor_roas = max(roas_meta, roas_google)
    media_ret   = round(sum(float(r[5] or 0) for r in reels_periodo)/len(reels_periodo),1) if reels_periodo else 0
    kpis = {
...
def ganchos(id):
    ...
    cur.execute('SELECT * FROM clientes WHERE id=?', (id,))
    cliente = cur.fetchone()
    cur.execute('SELECT * FROM ganchos WHERE cliente_id=? ORDER BY criado_em DESC', (id,))
    ganchos_list = cur.fetchall(); conn.close()
    return render_template('ganchos.html', cliente=cliente, ganchos=ganchos_list)
...
def novo_gancho(id):
    ...
    conn.commit(); conn.close()
    return redirect(f'/ganchos/{id}')

@app.route('/ganchos/excluir/<int:id>', methods=['POST'])
def excluir_gancho(id):
    if not logado(): return redirect('/login')
    conn = sqlite3.connect('banco.db'); cur = conn.cursor()
    cur.execute('SELECT cliente_id FROM ganchos WHERE id=?', (id,))
    row = cur.fetchone()
    cur.execute('DELETE FROM ganchos WHERE id=?', (id,))
    conn.commit(); conn.close()
    return redirect(f'/ganchos/{row[0]}' if row else '/')
...
def calendario(id):
    ...
    cur.execute('SELECT * FROM clientes WHERE id=?', (id,))
    cliente = cur.fetchone()
    cur.execute('SELECT * FROM eventos WHERE cliente_id=? ORDER BY data_evento', (id,))
    eventos_list = cur.fetchall()
    cur.execute('SELECT id, nome FROM clientes ORDER BY nome')
    todos_clientes = cur.fetchall()
    conn.close()
    return render_template('calendario.html', cliente=cliente,
                           eventos=eventos_list, todos_clientes=todos_clientes)
...
def novo_evento(id):
    ...
    cur.execute('INSERT INTO eventos (cliente_id,titulo,tipo,data_evento,hora,descricao) VALUES (?,?,?,?,?,?)',
        (id, request.form['titulo'], request.form['tipo'],
         request.form['data_evento'], request.form.get('hora',''), request.form.get('descricao','')))
    conn.commit(); conn.close()
    return redirect(f'/calendario/{id}')

@app.route('/calendario/excluir/<int:id>', methods=['POST'])
def excluir_evento(id):
    if not logado(): return redirect('/login')
    conn = sqlite3.connect('banco.db'); cur = conn.cursor()
    cur.execute('SELECT cliente_id FROM eventos WHERE id=?', (id,))
    row = cur.fetchone()
    cur.execute('DELETE FROM eventos WHERE id=?', (id,))
    conn.commit(); conn.close()
    return redirect(f'/calendario/{row[0]}' if row else '/')
...
def api_eventos(cliente_id):
    ...
    CORES = {'postagem':'#ec4899','review':'#3b82f6','metricas':'#f59e0b',
             'campanha':'#22c55e','reuniao':'#a78bfa','outro':'#64748b'}
    return jsonify([{
        'id':r[0], 'title':r[2], 'start':r[4]+('T'+r[5] if r[5] else ''),
        'backgroundColor':CORES.get(r[3],'#64748b'), 'borderColor':CORES.get(r[3],'#64748b'),
        'extendedProps':{'tipo':r[3], 'descricao':r[6] or ''}
    } for r in rows])


# ─── EDITAR / EXCLUIR MÉTRICAS ────────────────────────────────────────
...
def excluir_meta(id):
    ...
    conn.commit(); conn.close()
    return redirect(f'/cliente/{row[0]}' if row else '/')
...
def editar_meta(id):
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        row = cur.execute('SELECT cliente_id FROM metricas_meta WHERE id=?', (id,)).fetchone()
        conn.close(); return redirect(f'/cliente/{row[0]}')
    cur.execute('SELECT * FROM metricas_meta WHERE id=?', (id,))
...
def editar_google(id):
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        row = cur.execute('SELECT cliente_id FROM metricas_google WHERE id=?', (id,)).fetchone()
        conn.close(); return redirect(f'/cliente/{row[0]}')
    cur.execute('SELECT * FROM metricas_google WHERE id=?', (id,))
    m = cur.fetchone(); conn.close()
...
def editar_reels(id):
    ...
    if request.method == 'POST':
        ...
        conn.commit()
        row = cur.execute('SELECT cliente_id FROM metricas_reels WHERE id=?', (id,)).fetchone()
        conn.close(); return redirect(f'/cliente/{row[0]}')
    cur.execute('SELECT * FROM metricas_reels WHERE id=?', (id,))
    m = cur.fetchone(); conn.close()
    return render_template('editar_reels.html', m=m)


if __name__ == '__main__':from flask import Flask, render_template, request, redirect, session, jsonify
import sqlite3, hashlib, os, secrets
from datetime import datetime
from urllib.parse import urlencode
...
# ── SETUP (primeiro acesso sem usuarios) ──────────────────────────────
@app.route('/setup', methods=['GET', 'POST'])
def setup():
    conn  = sqlite3.connect('banco.db')
    total = conn.execute('SELECT COUNT(*) FROM usuarios').fetchone()[0]
    ...
    if request.method == 'POST':
        ...
        if u:
            ...
            html = f'''<div style="font-family:Arial,sans-serif;max-width:500px;margin:auto;
              ...
              <p style="font-size:11px;color:#334155;margin-top:24px;">Se nao foi voce, ignore.</p>
            </div>'''
...
def review(id):
    ...
    if google_semana:
        ac += 2 if float(google_semana[7] or 0) < 2 else (1 if float(google_semana[7] or 0) < 4 else 0)
    if reels_semana:
        # schema: id=0,cliente_id=1,data=2,nome_video=3,views=4,retencao=5,...
...
def relatorio(id):
    ...
    roas_meta   = max((float(m[6] or 0) for m in meta_periodo), default=0)
    roas_google = max((float(g[7] or 0) for g in google_periodo), default=0)
    # retencao no schema novo esta em index 5
    media_ret   = round(sum(float(r[5] or 0) for r in reels_periodo)/len(reels_periodo), 1) if reels_periodo else 0
    kpis = {
        'conversoes_total': conv_meta + conv_google,
